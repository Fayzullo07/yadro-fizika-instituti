...
const NewsSection: React.FC = () => {
  ...
  const cardColors: string[] = ['bg-blue-600', 'bg-green-600', 'bg-red-500', 'bg-purple-600'];
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                          ...
                          navigator.share({
                            ...
                            url:
                              typeof window !== 'undefined'
                                ? window.location.origin + `/news/${mainNews.id}`
                                : '',
                          });
                        }
                      }}
          ...
          <div className="lg:col-span-2 space-y-4">
            {/* Top News - Full Width */}
            {otherNews[0] && (
              <Link key={otherNews[0].id} to={`/news/${otherNews[0].id}`} className="block group">
                <div
                  className={`${cardColors[0]} rounded overflow-hidden shadow-lg hover:shadow-xl transition-all relative h-[285px]`}
                >
                  {/* Background Image */}
            ...
            <div className="grid grid-cols-2 gap-4">
              {otherNews.slice(1, 3).map((item, index) => (
                <Link key={item.id} to={`/news/${item.id}`} className="block group">
                  <div
                    className={`${cardColors[index + 1]} rounded overflow-hidden shadow-lg hover:shadow-xl transition-all relative h-[300px]`}
                  >
                    {/* Background Image */}
