...
import { useScientificArticles } from '@/hooks/usePublications';
import Loading from '@/components/shared/Loading/Loading';
import PageTitle from '@/components/shared/PageTitle/PageTitle';
import { ARTICLES_PATH } from '@/routes/path';

const Articles: React.FC = () => {
  const { t } = useLanguage();
  const { data, loading, error } = useScientificArticles();

  const items = data?.data ?? [];

  return (
    <div className="pb-10">
      <PageTitle>{t('nav.ilmiyFaoliyat.articles') || 'Ilmiy maqolalar'}</PageTitle>
